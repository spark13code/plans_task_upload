@extends('layout.layout')

@section('content')
            
<div class="main-panel">
          <div class="content-wrapper">
            <div class="row">
              <div class="col-sm-12">
              <div class="home-tab">

                <div class="d-sm-flex align-items-center justify-content-between border-bottom pb-2 mb-4">
                  
                <span class="fs-4 fw-bold">Eligibility Criteria</span>

                <a href="{{ route('eligible-criteria.add') }}" class="btn btn-primary text-white me-0"><i class="icon-plus"></i> Add Eligibility Criteria</a>
                   
                </div>


            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                   
                    <div class="table-responsive">
                      <table class="table table-hover" id="eligible-criteria">
                        <thead>
                          <tr>
                            <th>Name</th>
                            <th>Age Less Than</th>
                            <th>Age Grater Than</th>
                            <th>Last Login Days Ago</th>
                            <th>Income Less Than</th>
                            <th>Income Greater Than</th>
                            <th>Action</th>
                          </tr>
                        </thead>
                        <tbody>
                        </tbody>
                      </table>
                    </div>
                  </div>
                </div>
              </div>


              </div>
                  </div>
                </div>
              </div>
              </div>

@endsection

@section('js')
<script>
    $(document).ready(function () {
    $('#eligible-criteria').DataTable({
        processing: true, // Show processing indicator
        serverSide: true, // Enable server-side processing
        ajax: {
            url: '/get-eligible-criteria', // Endpoint to fetch data
            type: 'GET'
        },
        columns: [
            { data: 'name', name: 'name' },
            { data: 'age_less_than', name: 'age_less_than' },
            { data: 'age_greater_than', name: 'age_greater_than' },
            { data: 'last_login_days_ago', name: 'last_login_days_ago' },
            { data: 'income_less_than', name: 'income_less_than' },
            { data: 'income_greater_than', name: 'income_greater_than' },
            { data: 'action', name: 'action', orderable: false, searchable: false } // Action column
        ]
    });
});
</script>
@endsection